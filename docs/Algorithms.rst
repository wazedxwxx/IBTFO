...
IBTFO supports two options for time-stepping: a second-order explicit method-of-lines approach (MOL), and an iterative scheme base on a spectral deferred correction approach (SDC). Both time-steppers share a considerable amount of code.
...
geometries. Currently in IBTFO, there are 2 variants that can be
...
Currently in IBTFO Fortran source, there are 4 variants that can be
chosen through the ``ppm_type`` flag:
...
in IBTFO is a recollection of different extension of the PPM method published in Miller and Colella [JCP 2002] and Colella and Sekora [JCP 2008].
The actual implementation in IBTFO is described in the paper `Motheau and Wakefield, Investigation of finite-volume methods to capture shocks and turbulence spectra in compressible flows  [CAMCOS 2020]`, 
...
by the imposition of another limiter based on a flattening parameter to prevent artificial extrema in the reconstructed values. Here in IBTFO, the order of imposition
...
Note that with these definitions, there is no guarantee that :math:`\sum \boldsymbol{\mathcal{F}}_{m} = 0`, as required for mass conservation. An arbitrary *correction flux,* consistent with the mixture-averaged diffusion approximation, is added in IBTFO to enforce conservation.
