...
The IBTFO directory structure is as shown below:
...
Setting up a problem to run with IBTFO involves writing an input file and problem specific code in the run directory. 
IBTFO is built using the AMReX build system which supports out-of-source builds but as configured in Pele requires a specific directory structure. 
Within each case directory in Exec, are the source files that specify the setup of that particular case. 
The user has to build each case by compiling source files using a GNUMakefile which also compiles and links together AMReX and IBTFO sources.
The source files contained in the case directory are treated preferentially and can override IBTFO/AMReX source files.  
...
A list of available data in the Pele group can be found in IBTFO/Source/param_includes/IBTFO_params.H
